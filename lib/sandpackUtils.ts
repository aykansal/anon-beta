 ...
 */
export const getSandpackClient = async (previewRef: React.RefObject<SandpackPreviewRef | null>) => {
  try {
    ...
    toast.error('Failed to initialize preview: ' + (error instanceof Error ? error.message : 'Unknown error'));
    return null;
  }
};
 ...
 */
export const getCodeSandboxURL = async (client: unknown): Promise<CodeSandboxResult | null> => {
  try {
    console.log('Getting CodeSandbox URL...');
    // We need to use type assertion here as the SandPack client's type is not exported
    const result = await (client as { getCodeSandboxURL: () => Promise<CodeSandboxResult> }).getCodeSandboxURL();
    console.log('CodeSandbox URL result:', result);
    return result;
  } catch (error) {
    console.error('Error getting CodeSandbox URL:', error);
    toast.error('Failed to get CodeSandbox URL: ' + (error instanceof Error ? error.message : 'Unknown error'));
    return null;
  }
};
