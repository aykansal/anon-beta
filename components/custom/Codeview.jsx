...
// import { tail_kit } from '@/data/tail_kit';
...
const Codeview = ({ activeProject, isSaving, isGenerating, theme }) => {
    ...
    const fetchProjectCode = async (projectId) => {
      try {
        ...
        if (response.data) {
          ...
          console.log('Project code loaded:', response.data);
        }
      } catch (error) {
  ...
  useEffect(() => {
    // Setup event listener to provide current project files when requested
    const handleCurrentFilesRequest = () => {
      const currentFiles = {};

      // Use the current project files, not the default template
      if (currentProject && currentProject.codebase) {
        // Format the files as needed by backend
        ...
        // If no current project files, use defaults but mark them as templates
      ...
      // Dispatch event with current files
    ...
    // Also support the original template files request for compatibility
...
createRoot(document.getElementById('root')!).render(
  ...
  // '/src/vite-env.d.ts': `/// <reference types="vite/client" />`.trim(),
